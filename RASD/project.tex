...
\item \label{R26}The system must allow an agent to login, inserting its ID and password, blocking the registration if the data inserted is not correct.
\end{enumerate}
...
\item \label{R28}The system must allow all type of users (even if not authenticated) to select the see information for a city. The user can choose either the city where he is, using the GPS position, or an arbitrary selected location.
...
\item \label{R39}The system must allow a supervisor to login, inserting its ID and password,blocking the registration if the data inserted is not correct.
\end{enumerate}
...
=======
%\item \label{ref:maps-api} \hyperref[use:maps-api]{\textit{[MAPS-API]}} \href{https://developers.google.com/maps/documentation/geocoding/start?csw=1\#reverse}{\textit{https://developers.google.com/maps/documentation/geocoding/start?csw=1\#reverse}} - Google Maps API to obtain the address (including city name) from a GPS location
>>>>>>> fa6abe9698ea45333d5472ffc9529c10c777caeb
